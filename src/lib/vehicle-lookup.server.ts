...
} from "./vehicle-capabilities.server";
...
export type RegistrationVehicle = {
  ...
  source: "biluppgifter" | "transportstyrelsen";
};
...
export type RegistrationLookupResult = {
  ...
  estimate:
    | (ReturnType<typeof calculateTuning> & {
        brand: string;
        model: string;
        engine: string;
        fuel: Fuel;
...
export async function lookupAndEstimateRegistration(
  registration: string,
  stage: Stage,
  ...
  const capabilities = await resolveVehicleCapabilities(registration);
  ...
  if (!registryEngine) {
    ...
    return { vehicle, capabilities, match: null, estimate: null, reason };
  }
