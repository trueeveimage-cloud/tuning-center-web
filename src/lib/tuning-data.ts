...
> = {
  stage1: {
    dieselTurbo: { hp: 0.15, nm: 0.17 },
    bensinTurbo: { hp: 0.16, nm: 0.17 },
    sug: { hp: 0.03, nm: 0.03 },
  },
  stage2: {
    dieselTurbo: { hp: 0.23, nm: 0.24 },
    bensinTurbo: { hp: 0.25, nm: 0.25 },
    sug: { hp: 0.05, nm: 0.04 },
  },
};

function conservativeRange(estimate: number, stock: number) {
  const max = Math.max(stock, estimate - 5);
  return { min: Math.max(stock, max - 10), max };
}
...
export function calculateTuning(engine: Engine, stage: Stage) {
  ...
  // Keep estimates conservative: the calculated ceiling is reduced by 5 and shown as a range.
